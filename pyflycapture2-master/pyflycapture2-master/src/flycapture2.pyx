...
cdef class Context:
    ...
    def set_timestamping(self, on=True):
        cdef fc2Error r
        cdef fc2EmbeddedImageInfo eii
        with nogil:
            r = fc2GetEmbeddedImageInfo(self.ctx, &eii)
        raise_error(r)
        
        if eii.timestamp.available:
            eii.timestamp.onOff = on
        
        with nogil:
            r = fc2SetEmbeddedImageInfo(self.ctx, &eii)
        raise_error(r)
        
    def get_format7_configuration(self):
        cdef fc2Error r
        cdef fc2Format7ImageSettings f7is
        cdef unsigned int packet_size
        cdef float percentage
        
        with nogil:
            r = fc2GetFormat7Configuration(self.ctx, &f7is, &packet_size, &percentage)
        raise_error(r)
        
        f7is_p = {'mode':f7is.mode, 'offsetX':f7is.offsetX, 'offsetY':f7is.offsetY, 'width':f7is.width, 'height':f7is.height, 'pixel_format':f7is.pixelFormat}
        return (f7is_p, int(packet_size), float(percentage))

    def set_format7_configuration(self, img_settings, percentage):
        cdef fc2Error r
        cdef fc2Format7ImageSettings f7is
        cdef float p = percentage
        
        f7is.mode = img_settings['mode']
        f7is.offsetX = img_settings['offsetX']
        f7is.offsetY = img_settings['offsetY']
        f7is.width = img_settings['width']
        f7is.height = img_settings['height']
        f7is.pixelFormat = img_settings['pixel_format']
       
        with nogil:
            r = fc2SetFormat7Configuration(self.ctx, &f7is, p)
        raise_error(r)
...
cdef class Image:
    ...
    def get_timestamp(self):
        ...
        return {'cycle_secs':ts.cycleSeconds, 'cycle_count':ts.cycleCount}
